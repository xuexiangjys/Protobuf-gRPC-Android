...
public class XHttpApiTestFragment extends XPageFragment {
    ...
    @SingleClick
    @OnClick({R.id.btn_login, R.id.btn_get_user, R.id.btn_select_file1, R.id.btn_select_file2, R.id.btn_upload1, R.id.btn_upload2})
    ...
    private void testUpload2() {
        ...
        //构建服务api代理
        mStub = APIServiceGrpc.newStub(channel);
        mStub.uploadFile(builder.build(), new SimpleStreamObserver<XHttpApi.UploadFileReply>() {
            @Override
            protected void onSuccess(XHttpApi.UploadFileReply value) {
